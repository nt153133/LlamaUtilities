...
{
    [XmlElement("Snipe")]
    public class Snipe : ProfileBehavior
    {
        ...
        public override string StatusText { get { return "Sniping with " + QuestGiver; } }
            ...
            return new PrioritySelector(
                ...
                new Decorator(r => SnipeManager.addr != IntPtr.Zero, new PrioritySelector(
                    ...
                    new ActionRunCoroutine(async s => {
                        //get our target
                        var target = SnipeManager.SnipeObjects.FirstOrDefault(t => t.Hit == 0);
                        if (target.GameObject == null)
                        {
                            Logging.Write("We don't have a valid game object to snipe.");
                            return false;
                        }

                        var idx = SnipeManager.SnipeObjects.IndexOf(target);

                        await SnipeManager.Snipe((byte)idx);
                        return true;
                    })
                    )),
